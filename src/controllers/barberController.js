...
// Função para buscar o HISTÓRICO (Concluídos e Cancelados)
exports.getAppointmentHistory = async (req, res) => {
    ...
    try {
        const query = `
            SELECT a.id, a.appointment_time, a.status, s.name as service_name, c.name as client_name
            FROM appointments AS a
            JOIN services AS s ON a.service_id = s.id
            JOIN clients AS c ON a.client_id = c.id
            WHERE a.barber_id = $1 AND a.status IN ('Concluído', 'Cancelado')
